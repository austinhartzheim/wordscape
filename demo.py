#! /usr/bin/env python3
'''
Run wordscape on demo data to show what it can do and facilitate
testing by allowing easy repetition of operations on the same data.
'''
import argparse
import datetime
import wordscape


NODE_BASCOM_HILL = {
    'id': 349451685,
    'tag': {
        'ele': '290',
        'natural': 'peak',
        'wikipedia': 'en:Bascom Hill',
        'gnis:county_id': '025',
        'gnis:state_id': '55',
        'gnis:feature_id': '1842317',
        'name': 'Bascom Hill',
        'wikidata': 'Q4866334',
        'gnis:created': '06/01/1999'},
    'version': 5,
    'lon':-89.4036486,
    'timestamp': datetime.datetime(2015, 6, 29, 7, 4, 33),
    'changeset': 32277662,
    'visible': True,
    'user': 'Austin Hartzheim',
    'uid': 2110132,
    'lat': 43.0752828}

NODE_BUS_STOP = {
    'id': 461449913,
    'tag': {'highway': 'bus_stop', 'name': 'Linden at Henry'},
    'version': 2,
    'lon':-89.4102772,
     'timestamp': datetime.datetime(2009, 8, 11, 1, 32, 40),
     'changeset': 2103789,
     'visible': True,
     'user': 'sim',
     'uid': 35247,
     'lat': 43.0750935}

CHANGESET_WITH_DELTES = [{'type': 'way',
  'data': {'id': 431328206,
   'tag': {'highway': 'footway', 'surface': 'paved', 'footway': 'sidewalk'},
   'nd': [4305358726, 4305358741],
   'version': 2,
   'timestamp': datetime.datetime(2016, 7, 18, 4, 5, 31),
   'changeset': 40810093,
   'visible': True,
   'user': 'Austin Hartzheim',
   'uid': 2110132},
  'action': 'modify'},
 {'type': 'way',
  'data': {'id': 431328209,
   'tag': {'highway': 'footway', 'surface': 'paved', 'footway': 'sidewalk'},
   'nd': [4305358745, 4305358742],
   'version': 2,
   'timestamp': datetime.datetime(2016, 7, 18, 4, 5, 31),
   'changeset': 40810093,
   'visible': True,
   'user': 'Austin Hartzheim',
   'uid': 2110132},
  'action': 'modify'},
 {'type': 'way',
  'data': {'id': 431328207,
   'tag': {'highway': 'footway',
    'surface': 'paved',
    'wheelchair': 'yes',
    'footway': 'sidewalk'},
   'nd': [4305358727, 4305358742, 4305358743, 4305358744, 4305358741],
   'version': 2,
   'timestamp': datetime.datetime(2016, 7, 18, 4, 5, 32),
   'changeset': 40810093,
   'visible': True,
   'user': 'Austin Hartzheim',
   'uid': 2110132},
  'action': 'modify'},
 {'type': 'way',
  'data': {'id': 431328201,
   'tag': {'highway': 'footway', 'surface': 'paved', 'footway': 'sidewalk'},
   'nd': [4305358721,
    4305358722,
    4305358723,
    4305358724,
    4305358725,
    4305358726],
   'version': 2,
   'timestamp': datetime.datetime(2016, 7, 18, 4, 5, 32),
   'changeset': 40810093,
   'visible': True,
   'user': 'Austin Hartzheim',
   'uid': 2110132},
  'action': 'modify'},
 {'type': 'way',
  'data': {'id': 431328205,
   'tag': {'highway': 'footway', 'surface': 'paved', 'footway': 'sidewalk'},
   'nd': [4305358727,
    4305358728,
    4305358729,
    4305358730,
    4305358731,
    1904307635],
   'version': 2,
   'timestamp': datetime.datetime(2016, 7, 18, 4, 5, 32),
   'changeset': 40810093,
   'visible': True,
   'user': 'Austin Hartzheim',
   'uid': 2110132},
  'action': 'modify'},
 {'type': 'node',
  'data': {'id': 4279240359,
   'tag': {'cuisine': 'pizza', 'name': 'Papa Johns', 'amenity': 'fast_food'},
   'version': 2,
   'lon':-93.2187111,
   'timestamp': datetime.datetime(2016, 7, 18, 4, 5, 32),
   'changeset': 40810093,
   'visible': True,
   'user': 'Austin Hartzheim',
   'uid': 2110132,
   'lat': 44.9728046},
  'action': 'modify'},
 {'type': 'node',
  'data': {'id': 4306338491,
   'tag': {},
   'version': 2,
   'timestamp': datetime.datetime(2016, 7, 18, 4, 5, 32),
   'changeset': 40810093,
   'visible': False,
   'user': 'Austin Hartzheim',
   'uid': 2110132},
  'action': 'delete'},
 {'type': 'node',
  'data': {'id': 4306324597,
   'tag': {},
   'version': 2,
   'timestamp': datetime.datetime(2016, 7, 18, 4, 5, 32),
   'changeset': 40810093,
   'visible': False,
   'user': 'Austin Hartzheim',
   'uid': 2110132},
  'action': 'delete'}]



if __name__ == '__main__':
    node = wordscape.Node(NODE_BASCOM_HILL['id'], NODE_BASCOM_HILL)
    print('Bascom Hill:', node.identify())

    node = wordscape.Node(NODE_BUS_STOP['id'], NODE_BUS_STOP)
    print('Bus stop:', node.identify())

    changeset = wordscape.Changeset(40810093, CHANGESET_WITH_DELTES)
    print('--- Changeset: Bascom Hill ---\n', changeset.describe())
